...
export const TaskController = new Elysia({
	...
	.delete(
		...
		async ({ body, db, redis }: Context & { body: { taskID: string } }) => {
			const moneyClassService = new MoneyClassService(db, redis);
			const deleteMoneyActivity = new ActivityService(db, redis);
			...
			const deleteMoneyActivityLog = await deleteMoneyActivity.postActivity(
				body.taskID,
				$Enums.ActivityAction.REMOVED,
				"money from this task",
				"", // userId
			);
			WebSocket.broadcast("activity", deleteMoneyActivityLog);
			return Response.json("Success", { status: 200 });
		},
		{
			body: t.Object({
